	...
	<body>
		...
		<div id="show-country" class="content scaffold-show" role="main">
			...
			<ol class="property-list country">
				...
				<g:if test="${countryInstance?.city}">
				<li class="fieldcontain">
					<span id="city-label" class="property-label"><g:message code="country.city.label" default="City" /></span>
					
						<g:each in="${countryInstance.city}" var="c">
						<span class="property-value" aria-labelledby="city-label"><g:link controller="city" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
